...
# 测试
def test_model(model, test_loader,lr=0,log_name=None):
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    model.to(device)
    ...
    torch.save(model.state_dict(), f"ckpt/best_model_DeepGravity_{rmse_total:.4f}_{mae_total:.4f}_lr_{lr}.pth")


    all_real_od_t = np.concatenate(all_real_od, axis=0)
    all_pred_od_t = np.concatenate(all_pred_od, axis=0)
    np.save("../可视化/测试集TNN/Pred_DeepGravity_6.88.npy", all_pred_od_t)
...
def main():
    ...
    lr_list = [0.0035] # not MCM 是否当前最佳 YES 6.20
    # lr_list = [0.003]   # MCM 是否当前最佳 YES 6.20
    # 遍历学习率列表
    for lr in lr_list:
        print(f"当前学习率: {lr}")

        train_loader, val_loader, test_loader, log_filename = load_data(is_mcm=False)

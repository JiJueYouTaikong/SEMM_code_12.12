...
# 保存日志文件
log_filename = f"log/ANN_完整批处理调参_3.17.log"
...
def test_model(model, test_loader,lr=0):
    ...
    model.to(device)
    model.load_state_dict(torch.load("ckpt/best_model_feature1_25.1.14数据集版本.pth"))
    # model.load_state_dict(torch.load("ckpt/best_model_feature1_25.1.14数据集版本_8.3251_1.1967_留档.pth"))
    ...
    torch.save(model.state_dict(), f"ckpt/best_model_feature1_25.1.14数据集版本_{rmse_total:.4f}_{mae_total:.4f}_lr_{lr}.pth")

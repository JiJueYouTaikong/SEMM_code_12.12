...
def evaluate_all_methods():
    ...
    special_methods = {"OVS", "OVS-MCM", "DeepGravity", "DeepGravity-MCM", "DS", "DS-MCM", "GPT2", "GPT2-MCM", "SEMM",
                       "SEMM wo MCSM"}
    ...
    for method, pred in predictions_dict.items():
        ...
        # ====== 新增：平峰 / 高峰 指标计算 ======
        peak_idx = [4, 5, 18, 19, 28, 29]
        all_idx = set(range(35))
        off_idx = sorted(list(all_idx - set(peak_idx)))

        pred_peak = pred[peak_idx]
        pred_off = pred[off_idx]

        real_peak = real[peak_idx]
        real_off = real[off_idx]



        # 计算平峰指标
        rmse_off, mae_off, mape_off, cpc_off, jsd_off = evaluate_metrics(pred_off, real_off)
        # 计算高峰指标
        rmse_peak, mae_peak, mape_peak, cpc_peak, jsd_peak = evaluate_metrics(pred_peak, real_peak)

        print(
            f"    Off-peak -> RMSE={rmse_off:.4f}, MAE={mae_off:.4f}, MAPE={mape_off:.4f}, CPC={cpc_off:.4f}, JSD={jsd_off:.4f}")
        print(
            f"    Peak     -> RMSE={rmse_peak:.4f}, MAE={mae_peak:.4f}, MAPE={mape_peak:.4f}, CPC={cpc_peak:.4f}, JSD={jsd_peak:.4f}")
        print("-" * 60)

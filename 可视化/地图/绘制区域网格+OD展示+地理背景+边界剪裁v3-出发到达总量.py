...
# YlOrRd  RdBu  viridis  Accent
def get_color_map(values, cmap_name='Reds'):
    norm = mcolors.Normalize(vmin=values.min(), vmax=values.max())
    cmap = cm.get_cmap(cmap_name)
    return [mcolors.to_hex(cmap(norm(val))) if val > 0 else "None" for val in values]
...
for label, file_path in od_sources.items():
    ...
    # 输出地图
    draw_map(departure_colors, f"{label} - 总出发量", f"{label}_departure_map.html")
    draw_map(arrival_colors, f"{label} - 总到达量", f"{label}_arrival_map.html")

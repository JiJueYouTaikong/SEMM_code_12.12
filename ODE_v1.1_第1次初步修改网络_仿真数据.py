...
# 训练过程
def train_model(model, train_loader, val_loader, epochs=100, patience=10,learning_rate=0.001, device_id='cuda:3', load=0):


    if load == 1 :
        # model.load_state_dict(torch.load('ckpt_仿真/best_model_feature4_25.1.14数据集版本_8.4087_1.3267.pth'))
        model.load_state_dict(torch.load('ckpt_仿真/best_model_feature4_25.1.14数据集版本.pth'))
    ...
    for epoch in range(epochs):
        ...
        if val_loss < best_val_loss:
            ...
            patience_counter = 0
            # 保存最佳模型
            torch.save(model.state_dict(), "ckpt_仿真/best_model_feature4_25.1.14数据集版本.pth")
...
def load_data():
    ...
    # 加载数据
    speed = np.load('data/仿真_真实_总Speed矩阵_V1.1_25.2.26.npy')  # 形状 [T, N]


    od = np.load('data/仿真_真实_总OD矩阵_V1.1_25.2.26.npy')  # 形状 [T, N, N]
    
    
    production = np.sum(od, axis=-1)  # 形状 [T, N]
    ...
    speed_freq = np.array(speed_freq)
    od_freq = get_X_Freq(production)
    od_freq = np.array(od_freq)
...
# 测试
def test_model(model, test_loader,lr=0, device='cuda:3'):
    device = torch.device(device if torch.cuda.is_available() else "cpu")
    model.to(device)
    model.load_state_dict(torch.load("ckpt_仿真/best_model_feature4_25.1.14数据集版本.pth"))
    # model.load_state_dict(torch.load("ckpt_仿真/best_model_feature4_25.1.14数据集版本_4.9699_0.8671_lr_0.01.pth"))
    ...
    torch.save(model.state_dict(), f"ckpt_仿真/best_model_feature4_25.1.14数据集版本_{rmse_total:.4f}_{mae_total:.4f}_lr_{lr}.pth")
    ...
    print("-------------------不同时间步上---------------------")
    for i in range(all_real_od_t.shape[0]-36, all_real_od_t.shape[0]):
        print("-----真实值------")
...
def main():
    ...
    # 训练模型
    train_model(model, train_loader, val_loader, epochs=2000, patience=10, learning_rate=lr, device_id=device, load=0)

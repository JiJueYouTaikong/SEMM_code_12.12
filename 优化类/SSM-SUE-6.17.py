...
def logit_traffic_assignment(G, od_matrix_t, lambda_param):
    ...
    :param G: Graph结构
    :param od_matrix_t: 当前迭代的估计OD
    :param lambda_param: logit参数
    :return:
    '''
...
for i in range(num_samples):
    ...
    for epoch in range(num_epochs):
        ...
        # loss = traffic_volume_loss(flow_est, flow_tensor)


        if loss <= tol:
            print(f"Loss best:{loss}",flush=True)
            break

        # 反向传播和优化
        loss.backward()
        optimizer.step()
        print('[INFO] Epoch {}/{}, Loss: {:.4f}'.format(epoch, num_epochs, loss.item()),flush=True)
        print(f"[INFO] ODE:{model.ode.sum():.2f}, T: {od[i].sum():.2f}", flush=True)
    ...
    print(f"[INFO] 样本{i}的RMSE:{rmse:.4f} MAE:{mae:.4f} MAPE:{mape:.4f}",flush=True)
    rmse_total += rmse
    mae_total += mae
    mape_total += mape
...
print(f"[INFO] 所有预测的OD矩阵已保存",flush=True)

print(f"[INFO] Total Test loss: {loss_test:.4f}, RMSE: {rmse_test:.4f}, MAE: {mae_test:.4f}, MAPE: {mape_test:.4f}",flush=True)

...
def bpr(t0, v, c, alpha=0.15, beta=4):
    return t0 * (1 + alpha * (v / c) ** beta)


def precompute_paths(G, max_path_len=3):
    od_paths = {}
    N = G.number_of_nodes()
...
def logit_traffic_assignment_fast(od_matrix_t, od_paths, path_times_dict, G, dist_matrix, capacity, lambda_param):
    ...
    new_flows = np.zeros((N, N))
    for (origin, destination), paths in od_paths.items():
        demand = od_matrix_t[origin, destination]
        if demand <= 0 or origin == destination:
            continue
...
def run_dta_logit_fast(od_matrix, G, dist_matrix, lambda_param, max_iter, tol, capacity, od_paths, path_times_dict):
    # print("run_dta_logit_fast被调用")
    N = od_matrix.shape[0]
    prev_flow = np.zeros((N, N))

    for k in range(max_iter):
        new_flow = logit_traffic_assignment_fast(od_matrix, od_paths, path_times_dict, G, dist_matrix, capacity, lambda_param)
        # MSA update
        step = 1 / (k + 1)
        flow_est = (1 - step) * prev_flow + step * new_flow

        # 收敛判断
        diff = np.abs(flow_est - prev_flow).max()
        # print(f"    diff: {diff:.6f}",flush=True)
        if diff < tol:
            # print("     Converged.",flush=True)
            break

        prev_flow = flow_est.copy()
...
class LeastSquaresOptimizer:
    ...
    def forward_model(self, ode_flat):
        ...
        flow_pred = run_dta_logit_fast(ode_matrix,
                                       self.G,
                                       self.dist_matrix,
                                       self.lambda_param,
                                       self.max_iter,
...
for i in range(num_samples):
    ...
    # 预计算的OD路径
    od_paths = precompute_paths(G, max_path_len=3)
    ...
    optimizer = LeastSquaresOptimizer(N, init_matrix, G, dist_matrix,
                                      lambda_param=1, max_iter=max_iter, tol=tol, capacity=500,
                                      od_paths=od_paths, path_times_dict=path_times_dict)

    print("进入optimizer.optimize(flow[i])...",flush=True)
    optimized_ode = optimizer.optimize(flow[i])
    print("结束optimizer.optimize(flow[i])", flush=True)

    # 将当前样本的预测OD添加到列表
    all_predicted_od.append(optimized_ode)
...
# 将所有预测OD拼接为一个数组
all_predicted_od = np.array(all_predicted_od)

# 保存为npy文件
output_path = os.path.join(output_dir, f"Pred_SSM_LS_SUE_MSA_6.15.npy")
np.save(output_path, all_predicted_od)
print(f"所有预测OD矩阵已保存至: {output_path}")
print(f"保存的数组形状: {all_predicted_od.shape}")  # 形状应为 (num_samples, N, N)
...
print(f"Total Test RMSE: {rmse_test:.4f}, MAE: {mae_test:.4f}, MAPE: {mape_test:.4f}",flush=True)
...
log_filename = f"log/SSM-LS.log"
with open(log_filename, 'a') as log_file:
    log_file.write(
        f"Samples: {num_samples}, MaxIter: {num_epochs}, Tol: {tol}, Test RMSE: {rmse_test:.4f} MAE: {mae_test:.4f} MAPE: {mape_test:.4f}, Times: {times}\n",flush=True)

...
class ODModel(nn.Module):
    def __init__(self, N, temp, freq, num_layers=3, hidden_units=128, dropout_rate=0.1, ablate=None):
    ...
    def forward(self, x):
        ...
        # 创建特征输入，根据 ablate 参数决定是否保留
        x_temp = np.zeros((B, N)) if 'temp' in self.ablate else np.tile(self.temp, (B, 1))
        x_freq = np.zeros((B, N)) if 'freq' in self.ablate else np.tile(self.freq, (B, 1))
        if 'random' in self.ablate:
            x_random = np.zeros((B, N))
        else:
...
def train_model(model, train_loader, val_loader, epochs=100, patience=10, learning_rate=0.001, is_mcm=None, freq_method=None,stop_type=None):
    ...
    for epoch in range(epochs):
        ...
        if loss_value < best_val_loss:
            ...
            patience_counter = 0
            # 保存最佳模型
            torch.save(model.state_dict(), f"ckpt/v1_4/w-RE-wo-RE-SingleFactor_{freq_method}_is_mcm_{is_mcm}.pth")
...
def load_data(is_mcm=False,freq_shuffle=True,freq_method=None):
    ...
    log_filename = f"log/v1_4/w-RE-wo-RE-SingleFactor_{ablate[0]}_{freq_method}_MCM_{is_mcm}_Shuffle_{freq_shuffle}.log"
    set_seed(42)
...
def test_model(model, test_loader, lr: float,log_name=None,patience=30, is_mcm=None, freq_method=None,stop_type=None):
    device = torch.device("cuda:1" if torch.cuda.is_available() else "cpu")
    model.to(device)
    path = f"ckpt/v1_4/w-RE-wo-RE-SingleFactor_{freq_method}_is_mcm_{is_mcm}.pth"
...
def main():
    ...
    #            0.0005, 0.0004, 0.0003, 0.0002, 0.0001, 0.00005]
        ...
        for num_layers, hidden_units, dropout_rate in product(num_layers_list, hidden_units_list, dropout_list):
            for lr in lr_list:
                ...
                model = ODModel(
                    ...
                    ablate=ablate  # 可设置为 ['temp'], ['freq'], ['random'], 或组合
                )

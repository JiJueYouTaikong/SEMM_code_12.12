...
def train_model(model, train_loader, val_loader, test_loader,epochs, patience, learning_rate, lr2,test_lr,ft_epochs):
    ...
    for epoch in range(epochs):
        ...
        if val_loss < best_val_loss:
            ...
            patience_counter = 0
            # 保存最佳模型
            torch.save(model.state_dict(), "ckpt/best_model_feature1_25.1.14数据集版本_stage1.pth")
    ...
    model.load_state_dict(torch.load("ckpt/best_model_feature1_25.1.14数据集版本_stage1.pth"))
    best_val_loss = float('inf')
    patience_counter = 0

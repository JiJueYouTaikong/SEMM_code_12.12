...
class TODVolumeMapping(nn.Module):
    def __init__(self, od_dim, link_dim):
        ...
        self.fc_route = nn.Linear(od_dim, 128)
    ...
    def forward(self, g):
        p = self.sigmoid(self.fc_route(g))  # (batch_size, 128)
        p = p.unsqueeze(1)  # (batch_size, 1, 128)

        # 卷积层处理
        h1 = self.conv1(p)  # (batch_size, 32, 126)
        h2 = self.conv2(h1)  # (batch_size, 64, 124)

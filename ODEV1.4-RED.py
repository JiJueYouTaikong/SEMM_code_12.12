...
# 训练过程
def train_model(model, train_loader, val_loader, epochs=100, patience=10, learning_rate=0.001, is_mcm=None, freq_method=None,stop_type=None,device='cuda:3'):
...
def calculate_rmse_mae(predictions, targets):
    ...
    :param predic1.1km网格.htmltions: [T,N,N] T个时间步上的OD矩阵预测值
...
# 测试
def test_model(model, test_loader, lr: float,log_name=None,patience=30, is_mcm=None, freq_method=None,stop_type=None,device='cuda:3'):
    device = torch.device(device if torch.cuda.is_available() else "cpu")
    model.to(device)
    print(f"Using device: {device}")
    ...
    np.save('可视化/测试集TNN/Pred_RED-5.7127.npy',all_pred_od_t)
...
def parse_args():
    parser = argparse.ArgumentParser(description="Train ODModel with spectral features.")

    # 模型与训练参数
    parser.add_argument('--is_mcm', type=bool, default=True, help='是否使用MCM数据')
    parser.add_argument('--freq_method', type=str, default='WT', choices=['WT', 'STFT', 'FFT'], help='频域特征提取方法')
    parser.add_argument('--stop_type', type=str, default='nll', choices=['rmse', 'nll'], help='提前停止判据')
    parser.add_argument('--num_layers', type=int, default=8, help='MLP层数')
    parser.add_argument('--hidden_units', type=int, default=256, help='MLP隐藏层单元数')
    parser.add_argument('--dropout_rate', type=float, default=0.0, help='Dropout比例')
    parser.add_argument('--lr', type=float, default=0.0025, help='学习率')
    parser.add_argument('--patience', type=int, default=20, help='早停容忍轮数')
...
def main():
    ...
    param = (f"MCM={args.is_mcm}, Method={args.freq_method}, Stop={args.stop_type}, "
             f"Layers={args.num_layers}, Hidden={args.hidden_units}, "
             f"Dropout={args.dropout_rate}, LR={args.lr}")
    print(param)

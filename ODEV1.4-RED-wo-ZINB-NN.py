...
class ODModel_MLP(nn.Module):
    def __init__(self, N, temp, freq, num_layers=3, hidden_units=128, dropout_rate=0.1):
        ...
        layers = [nn.Linear(input_dim, hidden_units), nn.ReLU(), nn.Dropout(dropout_rate)]
        for _ in range(num_layers - 2):
            layers += [nn.Linear(hidden_units, hidden_units), nn.ReLU(), nn.Dropout(dropout_rate)]
        layers += [nn.Linear(hidden_units, output_dim)]
    ...
    def forward(self, x):
        ...
        out = self.mlp(weighted_sum).view(B, N, N)
        return out
    ...
    for epoch in range(epochs):
        ...
        with torch.no_grad():
            for data in val_loader:
                ...
                val_rmse += torch.sqrt(torch.mean((preds - targets) ** 2)).item()
        ...
        if epoch % 5 == 0:
            print(f"Epoch [{epoch + 1}/{epochs}], Train MSE: {train_loss:.4f}, Val MSE: {val_loss:.4f}, MAE: {val_mae:.4f}, RMSE: {val_rmse:.4f}")

        if stop_type == 'rmse':
            loss_value = val_rmse
        elif stop_type == 'nll' or stop_type == 'mse':
            loss_value = val_loss
        else:
            return -1
        ...
        if loss_value < best_val_loss:
            ...
            patience_counter = 0
            torch.save(model.state_dict(), f"ckpt/v1_4/wo-ZINB-NN_{freq_method}_is_mcm_{is_mcm}.pth")
            print(f"Best model saved at epoch {epoch + 1}, best val: {best_val_loss:.4f}")
        else:
            patience_counter += 1
...
def load_data(is_mcm=False,freq_shuffle=True,freq_method=None):
    ...
    log_filename = f"log/v1_4/wo-ZINB-NN-{freq_method}_MCM_{is_mcm}_Shuffle_{freq_shuffle}.log"
    set_seed(42)
...
def test_model(model, test_loader, lr: float, log_name=None, patience=30, is_mcm=None, freq_method=None, stop_type=None):
    device = torch.device("cuda:3" if torch.cuda.is_available() else "cpu")
    model.to(device)
    path = f"ckpt/v1_4/wo-ZINB-NN_{freq_method}_is_mcm_{is_mcm}.pth"
    ...
    with torch.no_grad():
        for data in test_loader:
            ...
            rmse, mae, mape, cpc, jsd = calculate_rmse_mae(preds * mask, targets)
            rmse_total += rmse
            mae_total += mae
            mape_total += mape
            cpc_total += cpc
            ...
            all_pred_od.append(preds.cpu().numpy() * mask.cpu().numpy())
    ...
    print(f"Test MSE: {test_loss:.4f}, RMSE: {rmse_total:.4f}, MAE: {mae_total:.4f}, MAPE: {mape_total:.4f}, CPC: {cpc_total:.4f}, JSD: {jsd_total:.4f}")

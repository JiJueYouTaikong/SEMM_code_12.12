...
# 保存日志文件
log_filename = f"log/v1_1/training_log_25.1.14版本_{time.strftime('%Y%m%d_%H%M%S')}.log"
...
def train_model(model, train_loader, val_loader, epochs=100, patience=10,learning_rate=0.001, load=0):
    ...
    if load == 1 :
        # model.load_state_dict(torch.load('ckpt/v1_1/best_model_feature4_25.1.14数据集版本_8.4087_1.3267.pth'))
        model.load_state_dict(torch.load('ckpt/v1_1/best_model_feature4_25.1.14数据集版本.pth'))
    ...
    for epoch in range(epochs):
        ...
        if val_loss < best_val_loss:
            ...
            patience_counter = 0
            # 保存最佳模型
            torch.save(model.state_dict(), "ckpt/v1_1/best_model_feature4_25.1.14数据集版本.pth")
...
def load_data():
    ...
    print(f"train_data: {train_data[0,66,:]}")
...
def test_model(model, test_loader,lr=0):
    ...
    model.to(device)
    # model.load_state_dict(torch.load("ckpt/v1_1/best_model_feature4_25.1.14数据集版本.pth"))
    model.load_state_dict(torch.load("ckpt/v1_1/best_model_feature4_25.1.14数据集版本_6.8681_1.1940_lr_0.015.pth"))
    ...
    torch.save(model.state_dict(), f"ckpt/v1_1/best_model_feature4_25.1.14数据集版本_{rmse_total:.4f}_{mae_total:.4f}_lr_{lr}.pth")

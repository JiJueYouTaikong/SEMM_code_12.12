...
# 训练过程
def train_model(model, train_loader, val_loader, epochs=100, patience=10, learning_rate=0.001,device_id=None, load=0):
    if load == 1:
        # model.load_state_dict(torch.load('ckpt_仿真/v1_2/best_model_feature4_25.1.14数据集版本_8.4087_1.3267.pth'))
        model.load_state_dict(torch.load('ckpt_仿真/v1_2/best_model_feature4_25.1.14数据集版本.pth'))
        print(f"best model loaded")

    device = device_id
    print("训练device:",device)
    ...
    for epoch in range(epochs):
        ...
        if val_loss < best_val_loss:
            ...
            patience_counter = 0
            # 保存最佳模型
            torch.save(model.state_dict(), "ckpt_仿真/v1_2/best_model_feature4_25.1.14数据集版本.pth")
...
def load_data():
    ...
    speed = np.load('data/仿真_真实_总Speed矩阵_V1.3_60days.npy')  # 形状 [T, N]
    od = np.load('data/仿真_真实_总OD矩阵_V1.3_60days.npy')  # 形状 [T, N, N]

    # speed = np.load('data/仿真_真实_总Speed矩阵_V1.2_25.2.26.npy')  # 形状 [T, N]
    # od = np.load('data/仿真_真实_总OD矩阵_V1.2_25.2.26.npy')  # 形状 [T, N, N]
    ...
    # train_size = int(T * 0.9923)
    # val_size = int(T * 0.00377)
    ...
    # print(f"train_data: {train_data[0, :2]}")
...
# 测试
def test_model(model, test_loader, lr: float,device):
    device = device
    print("测试device:",device)
    model.to(device)
    model.load_state_dict(torch.load("ckpt_仿真/v1_2/best_model_feature4_25.1.14数据集版本.pth"))
    # model.load_state_dict(torch.load("ckpt_仿真/v1_2/best_model_feature4_25.1.14数据集版本_6.8681_1.1940_lr_0.015.pth"))
    ...
    torch.save(model.state_dict(),
               f"ckpt_仿真/v1_2/best_model_feature4_25.1.14数据集版本_{test_loss:.4f}_{rmse_total:.4f}_{mae_total:.4f}_lr_{lr}.pth")
...
# 主程序
def main():
    parser = argparse.ArgumentParser(description="Train and test the ODModel.")
    parser.add_argument("--lr", type=float, help="Learning rate for training (default: 0.01)")
    ...
    # 训练模型
    train_model(model, train_loader, val_loader, epochs=2000, patience=20, learning_rate=lr, device_id=device, load=0)

    # 测试模型
    test_model(model, test_loader, lr=lr, device=device)

...
def process_sample(i, A, bounds, true_od, v_f, n_initial, max_iter, r):
    ...
    # ✅ 初始阶段估计 P 和 phi_hat，只估计一次
    P, phi_hat = estimate_M2_bayesian_multi_parallel(X, V)

    y = np.array([
        np.mean(np.abs(simulator(d, A) - v_f[i])[v_f[i] != 0] / v_f[i][v_f[i] != 0]) if (v_f[i] != 0).sum() > 0 else 0.0
        for d in X
    ])

    # 简化核函数设置
    kernel = C(1.0, (1e-3, 1e3)) * RBF(length_scale=1.0, length_scale_bounds=(1e-2, 1e2))
    gp = GaussianProcessRegressor(kernel=kernel, alpha=1e-6, normalize_y=True, n_restarts_optimizer=0)

    for iteration in range(max_iter):
        print(f"样本{i}的第{iteration}次迭代")
        gp.fit(X, y)

        # ✅ 改为候选点采样而非调用 minimize
        candidate_points = initial_sampling(5, bounds)
        acq_values = [
            acquisition_function(cand, gp, P, v_f[i], r, phi_hat)
            for cand in candidate_points
        ]
        d_new = candidate_points[np.argmin(acq_values)]

        non_zero_mask = v_f[i] != 0
        f_new = np.mean(np.abs(simulator(d_new, A) - v_f[i])[non_zero_mask] / v_f[i][
            non_zero_mask]) if non_zero_mask.sum() > 0 else 0.0
        X = np.vstack((X, d_new))
        y = np.append(y, f_new)
        # ✅ 不再更新 P 和 phi_hat，减少重复计算
...
def dynamic_OD_estimation(n_initial=5, max_iter=100):
    N = 1000

...
def propose_location(gp, bounds, P, v_f, r, phi_hat):
    phi_hat = np.maximum(phi_hat, 100.0)
    dim = bounds.shape[0]
...
def process_sample(i, A, bounds, true_od, v_f, n_initial, max_iter, r):
    ...
    d_opt = X[best_idx]
    f_opt = y[best_idx]
    # print(f"d_opt and true_od[i] shape:{d_opt.shape} \t {true_od[i].shape}")
    rmse, mae, mape = calculate_rmse_mae(d_opt, true_od[i])
    print(f"样本{i} Epoch：{iteration} RMSE:{rmse} MAE:{mae} MAPE:{mape}")
    return rmse, mae, mape


def dynamic_OD_estimation(n_initial=5, max_iter=10):
    N = 110*110

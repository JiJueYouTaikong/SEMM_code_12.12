...
class LSTMModel(nn.Module):
    ...
    def forward(self, x):
        # print(x.shape)
        x = x.view(x.size(0), x.size(1), -1)  # [batch_size, T, N*F]
...
def load_data():
    ...
    train_data = scaler.fit_transform(x_train.reshape(-1, 4)).reshape(-1, T, 110, 4)
    val_data = scaler.transform(x_val.reshape(-1, 4)).reshape(-1, T, 110, 4)
    test_data = scaler.transform(x_test.reshape(-1, 4)).reshape(-1, T,110, 4)

    # 1种特征
    train_data = torch.tensor(train_data[:,:,:,0], dtype=torch.float32)
    val_data = torch.tensor(val_data[:,:,:, 0], dtype=torch.float32)
...
def train_model(model, train_loader, val_loader, epochs=100, patience=10,learning_rate=0.001,load=0):
    ...
    if load == 1 :
        model.load_state_dict(torch.load('ckpt/best_model_1种特征_25.1.14数据集版本.pth'))
    ...
    # 4. 设置日志记录
    logging.basicConfig(filename='log/training_1种特征_25.1.14数据集版本.log', level=logging.INFO,
    ...
    for epoch in range(epochs):
        ...
        if val_loss < best_val_loss:
            ...
            torch.save(model.state_dict(), 'ckpt/best_model_1种特征_25.1.14数据集版本.pth')
...
def test_model(model, test_loader,learning_rate):
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    model.load_state_dict(torch.load('ckpt/best_model_1种特征_25.1.14数据集版本.pth'))
    # model.load_state_dict(torch.load('best_ckpt/best_model_四种特征_mlp_49.2769_5.9087.pth'))

    logging.info('load the best model, start testing')

    # 计算 RMSE, MAE 和 R²，并保存到日志
    total_rmse = 0.0
    total_mae = 0.0
    ...
    torch.save(model.state_dict(), f'ckpt/best_model_1种特征_25.1.14版本数据集_{rmse:.4f}_{mae:.4f}.pth')
    ...
    print(real_od_avg[60:68, 60:68].astype(int))
    print("--------------------------------------------------")
    print(predicted_od_avg[60:68, 60:68].astype(int))
...
def main():
    ...
    model = LSTMModel(input_size=110 * 1, hidden_size=256, output_size=110 * 110, num_layers=2)

    lr = 0.00001
    train_model(model, train_loader, val_loader, epochs=1000, patience=20, learning_rate=lr, load=1)

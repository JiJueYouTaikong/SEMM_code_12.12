...
def train_model(model, train_loader, val_loader, epochs=100, patience=10, learning_rate=0.001, hidden_size=0, load=0):
    if load == 1:
        model.load_state_dict(torch.load('ckpt/hidden512/best_model_4种特征_hidden512_25.1.14版本数据集.pth'))
    ...
    for epoch in range(epochs):
        ...
        if val_loss < best_val_loss:
            ...
            torch.save(model.state_dict(), 'ckpt/hidden512/best_model_4种特征_hidden512_25.1.14版本数据集.pth')
...
def test_model(model, test_loader, learning_rate):
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    model.load_state_dict(torch.load('ckpt/hidden512/best_model_4种特征_hidden512_25.1.14版本数据集.pth'))
    # model.load_state_dict(torch.load('ckpt/hidden512/best_model_4种特征_hidden512_25年2月17重新调参_25.1.14版本数据集_7.4021_1.1971_lr_0.02.pth'))
    ...
    torch.save(model.state_dict(),
               f'ckpt/hidden512/best_model_4种特征_hidden512_25.1.14版本数据集_{rmse:.4f}_{mae:.4f}_lr_{learning_rate}.pth')
...
def main():
    ...
    lr = 0.02  # best 0.02
    train_model(model, train_loader, val_loader, epochs=1000, patience=20, learning_rate=lr, load=0)

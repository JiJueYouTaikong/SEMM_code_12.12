...
class GRUModel(nn.Module):  # Changed from LSTMModel to GRUModel
    ...
    def forward(self, x):
        ...
        gru_out, hn = self.gru(x)  # [B,T,N*F] --> [B, T, hidden]
...
log_filename = f"log/GRU完整批处理的调参.log"
...
def main():
    ...
    for lr in lr_list:
        ...
        model = GRUModel(input_size=110, hidden_size=256, output_size=110 * 110, num_layers=2)  # Changed to GRUModel

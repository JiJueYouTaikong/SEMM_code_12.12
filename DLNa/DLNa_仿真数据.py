...
# 训练过程
def train_model(model, train_loader, val_loader, epochs=100, patience=10, learning_rate=0.001, device_id='cuda:3',
                load=0):
    if load == 1:
...
def load_data():
    ...
    # 加载数据
    speed = np.load('../data/仿真_真实_总Speed矩阵_V1.0_25.2.18.npy')  # 形状 [T, N]

    od = np.load('../data/仿真_真实_总OD矩阵_V1.0_25.2.18.npy')  # 形状 [T, N, N]
...
def test_model(model, test_loader, lr=0, device='cuda:3'):
    ...
    model.to(device)
    model.load_state_dict(torch.load("ckpt_仿真/best_model_feature4_25.1.14数据集版本.pth"))
    # model.load_state_dict(torch.load("ckpt_仿真/best_model_feature4_25.1.14数据集版本_8.3251_1.1967_留档.pth"))
    ...
    torch.save(model.state_dict(),
               f"ckpt_仿真/best_model_feature4_25.1.14数据集版本_{rmse_total:.4f}_{mae_total:.4f}_lr_{lr}.pth")
    ...
    print("-------------------不同时间步上---------------------")
    for i in range(0, all_real_od_t.shape[0], 350):
        print("-----真实值------")
...
def main():
    ...
    # 测试模型
    test_model(model, test_loader, device=device, lr=lr)

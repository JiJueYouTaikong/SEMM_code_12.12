...
def data_process_all(degree_to_km , grid_size_km, cent, T_speed, chunksize, boundary_path, od_path, speed_path, dataset_path,is_test):
    ...
    for chunk in pd.read_csv(od_path, encoding='utf-8', chunksize=chunksize, on_bad_lines='skip'):
        ...
        # print(f"正在处理第{chunk_count}块数据")
        #

        # # 对每个分块进行匹配操作
        # chunk['start_region_id'] = chunk.apply(
        #     lambda row: match_region_id(row['start_center_lnt'], row['start_center_lat'], grid_data), axis=1
        # )
        # chunk['end_region_id'] = chunk.apply(
        #     lambda row: match_region_id(row['end_center_lnt'], row['end_center_lat'], grid_data), axis=1
        # )
        #
        # # 筛选出同时匹配成功的数据
        ...
        # total_rows += len(filtered_chunk)  # 更新总行数
    ...
    # od_with_grid_np = np.array(od_with_grid)
    # np.save('批量处理的最终数据集/od_with_grid.npy', od_with_grid_np)

    # # 直接加载处理好的
    # od_with_grid = np.load('批量处理的最终数据集/od_with_grid_full.npy',allow_pickle=True)
    # od_with_grid = pd.DataFrame(od_with_grid)
    # print(f"OD数据网格映射完成")
    ...
    # 新的代码
    # 找出共有的区域ID并按升序排列
    # common_region_ids = sorted(np.intersect1d(speed_index, od_index))
    #
    # # 获取OD数据中共同区域ID对应的原始索引，并按common_region_ids顺序排列
    # od_indices = [np.where(od_index == rid)[0][0] for rid in common_region_ids]
    # # 获取Speed数据中共同区域ID对应的原始索引，并按common_region_ids顺序排列
    # speed_indices = [np.where(speed_index == rid)[0][0] for rid in common_region_ids]
    #
    # # 筛选并重新排序OD和Speed的数据，确保区域顺序一致
    # filtered_speed = speed[:, speed_indices, :]
    # filtered_od = od[:, od_indices, :][:, :, od_indices]
...
# speed, od = data_process_all(degree_to_km , grid_size_km, cent, T_speed, chunksize, boundary_path, od_path, speed_path, dataset_path,is_test)
